...
WayPointManager::WayPointManager(ros::NodeHandle nh, ros::NodeHandle pnh) {
  ...
  target_pose_subscriber = nh.subscribe("target_pose", 10, &WayPointManager::TargetPoseCallback, this);
...
void WayPointManager::ReadWayPointFile(const std::string a_file_name) {
  std::ifstream wp_file(a_file_name);
  if (!wp_file) {
  ...
  while (1) {
    ...
    WayPoint tmp_wp;
    line_ss >> tmp_wp.x >> tmp_wp.y >> tmp_wp.yaw >> tmp_wp.velocity >> tmp_wp.wp_type;
    tmp_wp.id = id;
    id += 1;
...
std::list<WayPoint>::iterator WayPointManager::SearchNearWaypointIterator(const WayPoint &target_wp) {
  const auto most_near_iter = std::min_element(std::begin(wp_array), std::end(wp_array), [&target_wp](WayPoint lhs,
                                                                                                      WayPoint rhs) {
...
void WayPointManager::CallBackInitialPose(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &initial_pose) {
  if (use_initial_pose_flag == false) return;
